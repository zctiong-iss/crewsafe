...
function validateVersionLabel(draft: Partial<PolicyVersionCreateRequest>, errors: FieldErrors): void {
...
function validateSource(draft: Partial<PolicyVersionCreateRequest>, errors: FieldErrors): void {
...
function validateEffectiveDate(draft: Partial<PolicyVersionCreateRequest>, errors: FieldErrors): void {
...
function validateThresholdGroup(
  group: (typeof THRESHOLD_GROUPS)[number],
  draft: Partial<PolicyVersionCreateRequest>,
  errors: FieldErrors,
): void {
...
export function validatePolicyVersion(draft: Partial<PolicyVersionCreateRequest>): FieldErrors {
  ...
  validateVersionLabel(draft, errors);
  validateSource(draft, errors);
  validateEffectiveDate(draft, errors);
  for (const group of THRESHOLD_GROUPS) {
    validateThresholdGroup(group, draft, errors);
  }
  validateEmergencyStop(draft, errors);
