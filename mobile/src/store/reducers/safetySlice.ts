 ...
 */
/**
 * The site to ask the live endpoints about: the one the worker's shift is on.
 *
 * This used to resolve the site from `GET /api/v1/sites` instead, because `fetchMyShift` was a
 * fixture whose site id (`11111111-…`) no deployment had, and every site-scoped endpoint answers
 * 403 for an id it does not recognise. SCRUM-266 made `/shifts/me` real, so the shift now names
 * a site that exists — and the workaround collapses to reading it, exactly as its note said it
 * would.
 *
 * The difference is not only tidiness. "First accessible site" was a guess that happened to be
 * right for a worker with one membership; this is the site they are actually standing on.
 */
async function loadConditions(
  ...
  const weather = await fetchSiteWeather(shift.siteId, workerId);
