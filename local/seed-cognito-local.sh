...
# chr(10) rather than a "\n" literal. This script has been written through a heredoc more
# than once, and an escape sequence there gets interpreted rather than preserved — which is
# precisely how this generator was silently broken: the string ended up split across real
# newlines, python exited with a SyntaxError, and `set -e` aborted the whole run right after
# backend-env.sh was written. The symptom was a missing .ps1 and no printed instructions.
io.open(target, "w", encoding="utf-8").write(chr(10).join(lines) + chr(10))
PYEOF
