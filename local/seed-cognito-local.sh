...
# This script uses associative arrays (declare -A), a bash 4.0 feature. macOS ships bash 3.2
# frozen at 2007 as /bin/bash — which #!/usr/bin/env bash resolves to unless a newer bash is
# earlier on PATH — so on a stock Mac it would die with "declare: -A: invalid option". Rather
# than depend on every teammate's PATH order, re-exec under a modern bash if we're on < 4.
if [[ "${BASH_VERSINFO[0]:-0}" -lt 4 ]]; then
  for newer_bash in /opt/homebrew/bin/bash /usr/local/bin/bash; do
    [[ -x "$newer_bash" ]] && exec "$newer_bash" "$0" "$@"
  done
  echo "This script needs bash 4+ (declare -A). Install one: brew install bash" >&2
  exit 1
fi
...
while (($#)); do
  case "$1" in
    ...
    --env)
      WRITE_ENV="${2:-}"
      [[ "$WRITE_ENV" == "password" || "$WRITE_ENV" == "hosted-ui" || "$WRITE_ENV" == "web" ]] || {
        echo "--env takes 'password', 'hosted-ui', or 'web'." >&2; exit 1; }
      shift 2 ;;      
    -h|--help) usage; exit 0 ;;
...
# Resolve the Python interpreter by a name that is guaranteed to exist. PEP 394 says only
# `python3` is safe to assume: a bare `python` may be 2.x, 3.x, or absent (a macOS python.org
# install provides python3 and no `python` at all). Prefer python3, fall back to python.
...
lines = [r"# Generated by seed-cognito-local.sh. Dot-source from the repo root:",
         r"#   . .\local\.cognito-local\backend-env.ps1"]
...
── 2c. web/.env.local — Cognito (PKCE), in the browser ────────────────────────────────
localhost, not 10.0.2.2: the browser is on this machine. The web client is the SAME
local-web client the backend already trusts (APP_COGNITO_CLIENT_IDS).

VITE_COGNITO_AUTHORITY=http://localhost:$PORT/$POOL
VITE_COGNITO_CLIENT_ID=$WEB_CLIENT
VITE_COGNITO_HOSTED_UI_DOMAIN=http://localhost:$PORT
VITE_REDIRECT_URI=http://localhost:5173/callback
VITE_POST_LOGOUT_REDIRECT_URI=http://localhost:5173/
VITE_API_BASE_URL=http://localhost:8080
VITE_COGNITO_LOCAL_ISSUER=http://cognito-local/$POOL
...
# ── Writing mobile/.env and web/.env ─────────────────────────────────────────────────────────────────
...
# Rewrite one env file in place from the newline-separated KEY=VALUE pairs in $ENV_PAIRS,
# copying from its .example first if absent and keeping a .bak. Env-file-agnostic: the same
# Python serves mobile/.env and web/.env.local, so neither the rewriter nor its heredoc is
...
# The web block is identical for every mode and — critically — includes
# VITE_COGNITO_LOCAL_ISSUER. The web app's dev-only auth override needs it: cognito-local
# mints tokens whose iss is http://cognito-local/$POOL, not the authority host, and without
# this line the override never activates and sign-in fails at "Could not reach the sign-in
# page".
WEB_PAIRS="VITE_COGNITO_AUTHORITY=http://localhost:$PORT/$POOL
VITE_COGNITO_CLIENT_ID=$WEB_CLIENT
VITE_COGNITO_HOSTED_UI_DOMAIN=http://localhost:$PORT
VITE_REDIRECT_URI=http://localhost:5173/callback
VITE_POST_LOGOUT_REDIRECT_URI=http://localhost:5173/
...
if [[ -n "$WRITE_ENV" ]]; then
  ...
  case "$WRITE_ENV" in
    web)
      ENV_PAIRS="$WEB_PAIRS"
      rewrite_env "web/.env.local" "web/.env.example"
      WROTE_WEB=true
      ;;
    hosted-ui)
...
EXPO_PUBLIC_COGNITO_HOSTED_UI_DOMAIN=http://localhost:$PORT
EXPO_PUBLIC_COGNITO_WEB_CLIENT_ID=$WEB_CLIENT"
      rewrite_env "mobile/.env" "mobile/.env.example"
      WROTE_MOBILE=true
      ;;
    password)
...
EXPO_PUBLIC_COGNITO_CLI_CLIENT_ID=$CLIENT"
      rewrite_env "mobile/.env" "mobile/.env.example"
      WROTE_MOBILE=true
      # --restart implies --env password AND sets up the whole local stack, so write the web
      # env too — from the SAME pool this run just seeded. Doing both in one invocation is
      # the point: a separate `--env web` run would mint a new pool and leave the backend and
      # browser on different pools, so sign-in would 401 at /api/v1/me.
  ...
  echo
  [[ "$WROTE_MOBILE" == true ]] && echo "  Metro must be restarted to pick this up: cd mobile && npm run start:clear"
  [[ "$WROTE_WEB" == true ]] && echo "  Vite must be restarted to pick this up:  cd web && npm run dev"
fi
