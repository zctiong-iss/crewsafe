...
[[ "$REPOSITORY" =~ ^[0-9]{12}\.dkr\.ecr\.ap-southeast-1\.amazonaws\.com/crewsafe/ml-service$ ]] || { echo "Unexpected repository" >&2; exit 1; }
...
actual_digest="$(aws ecr describe-images --repository-name crewsafe/ml-service --image-ids imageTag="$IMAGE_TAG" --query 'imageDetails[0].imageDigest' --output text)"
...
jq --arg image "$REPOSITORY@$IMAGE_DIGEST" '
  ...
  | .containerDefinitions |= map(if .name == "ml-service" then .image=$image else . end)
  | if ([.containerDefinitions[] | select(.name == "ml-service")] | length) == 1 then . else error("missing ml-service container") end
...
if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then printf '## ml-service staging deployment\n- Commit SHA: `%s`\n- Image digest: `%s`\n- Task definition: `%s`\n- Result: promoted (backend container image unchanged)\n' "$IMAGE_TAG" "$IMAGE_DIGEST" "$task_definition" >>"$GITHUB_STEP_SUMMARY"; fi
