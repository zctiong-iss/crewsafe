...
findings_missing_source_url="$work/findings-missing-source-url.json"
printf '{"Findings":[{"Id":"crewsafe/sonarcloud/zctiong-iss_crewsafe/SAFE-OPEN-1","ProductArn":"arn:aws:securityhub:ap-southeast-1:123456789012:product/123456789012/default","UpdatedAt":"2000-01-01T00:00:00Z","RecordState":"ACTIVE"}]}' >"$findings_missing_source_url"
repair_input="$work/repair-input.json"
jq '.valid | .issues[0].creationDate = "2000-01-01T00:00:00Z" | .issues[0].updateDate = "2000-01-01T00:00:00Z"' "$FIXTURES/active-cases.json" >"$repair_input"
findings="$findings_missing_source_url"
assert_exit 0 "existing finding without SourceUrl is repaired" run_import "$repair_input" "$empty" "$import_ok" "$out"
assert_contains "$(cat "$out")" "UPDATED" "SourceUrl repair is text-labelled"
repair_calls="$(cat "$work/calls.log")"
assert_contains "$repair_calls" '"sourceUrl":"https://sonarcloud.io/project/issues?id=zctiong-iss_crewsafe&open=SAFE-OPEN-1"' "repair payload includes the validated SonarCloud issue deep link"
assert_contains "$repair_calls" '"sourceUpdatedAt":"2000-01-01T00:00:00Z"' "repair payload preserves the Sonar source timestamp"
assert_not_contains "$repair_calls" '"updatedAt":"2000-01-01T00:00:00Z"' "repair payload uses a later provider timestamp"

findings_with_source_url="$work/findings-with-source-url.json"
printf '{"Findings":[{"Id":"crewsafe/sonarcloud/zctiong-iss_crewsafe/SAFE-OPEN-1","ProductArn":"arn:aws:securityhub:ap-southeast-1:123456789012:product/123456789012/default","UpdatedAt":"2000-01-01T00:00:00Z","SourceUrl":"https://sonarcloud.io/project/issues?id=zctiong-iss_crewsafe&open=SAFE-OPEN-1","RecordState":"ACTIVE"}]}' >"$findings_with_source_url"
