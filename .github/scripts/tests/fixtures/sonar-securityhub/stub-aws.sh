...
case "$service $operation" in
  ...
  "securityhub batch-import-findings")
    for arg in "$@"; do
      if [[ "$arg" == file://* ]]; then
        jq -c '.[0] | {providerSeverity: .FindingProviderFields.Severity, sonarSeverity: .ProductFields["crewsafe/sonarSeverity"], createdAt: .CreatedAt, updatedAt: .UpdatedAt}' "${arg#file://}" >>"${MOCK_CALL_LOG:?}"
      fi
    done
