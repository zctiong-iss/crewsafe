#!/usr/bin/env bash
# Validate a reviewed Trivy exception source file for any in-scope image.
# The source format is intentionally narrow and fail-closed:
#   CVE-YYYY-NNNN # owner:<token> exp:YYYY-MM-DD reason:<bounded text>
set -euo pipefail
...
[[ $# -eq 1 ]] || fail "usage: validate-trivy-exceptions.sh <source-file>"
SOURCE_FILE="$1"
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  [[ "$line" =~ owner:([^[:space:]#]+) ]] \
    || fail "line $line_number is missing owner:"
  [[ "$line" =~ reason:([^[:space:]#]) ]] \
    || fail "line $line_number is missing reason:"
  ...
  [[ "$exp_date" > "$TODAY" ]] \
    || fail "line $line_number has expired or expires today"
done <"$SOURCE_FILE"
