...
GITHUB_OUTPUT="" CREWSAFE_AWS_ACCOUNTS_JSON="$registry" "$ROOT/.github/scripts/terraform/resolve-terraform-account.sh" alice >/dev/null
if GITHUB_OUTPUT="" CREWSAFE_AWS_ACCOUNTS_JSON="$registry" "$ROOT/.github/scripts/terraform/resolve-terraform-account.sh" unknown >/dev/null 2>&1; then exit 1; fi
...
policy_account_output="$(GITHUB_OUTPUT="" CREWSAFE_AWS_ACCOUNTS_JSON="$policy_registry" "$ROOT/.github/scripts/terraform/resolve-terraform-account.sh" alice)"
jq -e '
...
for invalid in \
  ...
  if GITHUB_OUTPUT="" CREWSAFE_AWS_ACCOUNTS_JSON="$invalid" \
