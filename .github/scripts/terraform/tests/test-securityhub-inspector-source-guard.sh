...
jq -e '[.Statement[] | select(.Sid == "ManageEcrEnhancedScanning") | .Action | sort | . == ["ecr:DescribeRegistry","ecr:GetRegistryScanningConfiguration","ecr:PutRegistryScanningConfiguration"]]' "$ROOT/$apply_policy" >/dev/null
...
jq -e '[.Statement[] | select(.Sid == "ManageSecurityHubInsight") | .Action | sort | . == ["securityhub:CreateInsight","securityhub:DeleteInsight","securityhub:GetInsight","securityhub:GetInsights","securityhub:ListInsights","securityhub:UpdateInsight"]]' "$ROOT/$apply_policy" >/dev/null

jq -e '[.Statement[] | select(.Sid == "ReadSecurityHubInspector") | .Action | sort | . == ["ecr:DescribeRegistry","ecr:GetRegistryScanningConfiguration","inspector2:BatchGetAccountStatus","inspector2:ListCoverage","securityhub:DescribeHub","securityhub:GetInsight","securityhub:GetInsights","securityhub:ListInsights"]]' "$ROOT/$plan_policy" >/dev/null
