...
/**
 * Authorization behaviour of the SCRUM-317 action-status SSE endpoint: reuses {@code
 * @siteAccess} and the same SUPERVISOR/SAFETY_MANAGER/ADMIN role gate as {@link
 * com.crewsafe.conditions.api.SiteConditionsController#stream}, so it must be denied and
 * allowed in exactly the same shapes.
 *
 * @author Jemilin Beulah
 */
@AutoConfigureMockMvc
class ActionStatusStreamAuthorizationTest extends AbstractIntegrationTest {

    @DynamicPropertySource
    static void shortStreamTiming(DynamicPropertyRegistry registry) {
        // Short-circuits the production 10s/5min defaults so a leftover emitter doesn't linger.
        registry.add("app.action-dispatch.stream.push-interval", () -> "200ms");
        registry.add("app.action-dispatch.stream.emitter-timeout", () -> "2s");
    }
    ...
    private AppUser user(Role role) {
        String username = "action-status-authz-" + UUID.randomUUID();
        createCognitoUser(username);
        return users.save(new AppUser(username, subFor(username), "Action Status Authz Test " + role, role));
    }

    private Site site(String label) {
        return sites.save(new Site("Action Status Authz " + label + " " + UUID.randomUUID(),
    ...
    private String streamUrl(Site site) {
        return "/api/v1/sites/" + site.getId() + "/actions/stream";
    }
    ...
    @Test
    void workerIsForbiddenFromTheActionStatusStreamEvenAtTheirOwnSite() throws Exception {
    ...
    void unknownSiteIdIsForbiddenNotNotFound() throws Exception {
        mockMvc.perform(get("/api/v1/sites/" + UUID.randomUUID() + "/actions/stream")
